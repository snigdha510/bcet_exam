...
def create_new_quiz_post():
    ...
    # Check if the file has a name and is of JSON type
    if file.filename == '':
    ...
    if file and file.filename.endswith('.json'):
        try:
            ...
            flash(f'Error decoding JSON file: {e}', 'error')
            return redirect(request.url)
    else:
        flash('Invalid file format. Please upload a JSON file.', 'error')
        return redirect(request.url)

    # Extracting quiz details from JSON data
    quiz_title = data.get('quiz_title', '')
    start_time = data.get('start_time', '')
    end_time = data.get('end_time', '')
    questions = data.get('questions', [])

    if not (quiz_title and start_time and end_time and questions):
        flash('JSON data is missing required fields', 'error')
        return redirect(request.url)

    start_time = datetime.strptime(start_time, '%Y-%m-%d')
    end_time = datetime.strptime(end_time, '%Y-%m-%d') + timedelta(days=1) - timedelta(seconds=1)

    # Creating the quiz
    quiz = Quiz(title=quiz_title, start_time=start_time, end_time=end_time, teacher_id=current_teacher.id)

    total_marks = 0

    # Adding questions to the quiz
    for question_data in questions:
        question_desc = question_data.get('question_desc', '')
        ...
        question = Quiz_Questions(question_desc=question_desc, option_1=option_1, option_2=option_2,
                                  option_3=option_3, option_4=option_4, marks=marks)
        question.quiz = quiz
        db.session.add(question)
